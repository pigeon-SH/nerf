...
def positional_encoding(x, L=10):
    """ x shape: (batch_size, sample_num, 3)
    """
...
def volume_rendering(t, sigma, rgb):
    """ t shape: (batch_size, sample_num, 1)
        sigma shape: (batch_size, sample_num, 1)
        rgb shape: (batch_size, sample_num, 3)
    """
    delta = t[:, 1:, :] - t[:, :-1, :]  # shape: (batch_size, sample_num-1, 1)
    delta = torch.cat([delta, 1e10 * torch.ones_like(delta[:, 0:1, :])], dim=1)    # shape: (batch_size, sample_num, 1), inf for last distance
    cs = torch.cumsum(delta * sigma, dim=1) # shape: (batch_size, sample_num, 1)
    T = torch.exp(-cs)
